...
into an array of machine readable PIO instructions by the pioasm program, which is taken from the SDK. Its output contains some SDK specific code which we trim off to get an include file
ws2812d.pio.h, which contains the instructions, together with the addresses 'wrap' and 'wrap_target', which are labels essentially of the pio program.
...
high and 3 cycles low. The main program initializes the PIO peripheral and the XOSC clock, and sets pin 0 as PIO output. Then the instructions are loaded into the instruction registers of the PIO
