...
padding the resulting file with zeroes and adding a checksum (required by the bootrom), resulting in a 256-byte file which needs to be placed at the start of flash. To show that there is no magic
involved, I have taken the necessary parts from the SDK (some assembly code in the present directory, mainly register definitions in directory ../sdk\_include and the pad\_checksum program in ../utils)
